...
//		}
	...
	function nextSlide(navSlide){
		...
		if(!$('.imgLoaded',slide).length){
			...
			var url =  imgUrl +"?"+ new Date().getTime();
			imgLoaded.src =url;
			$(imgLoaded).data({loadedUrl:url});
			...
			if (!$(imgLoaded).get(0).complete || wT == '0' || hT == '0' || typeof wT === 'undefined' || wT === false || typeof hT === 'undefined' || hT === false) {
				...
				imgLoaded.onload = function() {
					...
					if(imgLoaded.src.indexOf($(imgLoaded).data('loadedUrl'))>0){
						target.find('.cameraSlide_'+slideI).hide().css('visibility','visible');
					}
					resizeImage();
					console.log('[src='+imgLoaded.src);
					console.log(']loaded='+$(imgLoaded).data('loadedUrl'));
					if(imgLoaded.src.indexOf($(imgLoaded).data('loadedUrl'))>0){
						console.log("equal");
						nextSlide(slideI+1);
					}
